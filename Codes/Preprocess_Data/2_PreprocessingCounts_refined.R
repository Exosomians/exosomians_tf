...
=======
COUNTS_DATA_DIR = 'Data/oldDataRefined/Counts_and_Beds/'
>>>>>>> c41dcf8ca76f890df1d189b59abb2b29f381c18c
...
countsFiles <- lapply(countsFiles, function(aFile) {
  aFile <- subset(aFile, select=c(V1, V2, V3, V4, V6, V7, V8))
  colnames(aFile) = c('chr', 'start','end', 'id', 'strand' ,'coverage', 'length')
...
>>>>>>> c41dcf8ca76f890df1d189b59abb2b29f381c18c
  ...
  aFile <- aFile[!duplicated(aFile$id),]
})


## checking if the regions are in order
...
designMat <- read.csv('Data/oldDataRefined/DesignMatrices/2_PrimaryDesignMat_label.csv', stringsAsFactors = F)
dim(designMat)
...
pl1=ggplot(designMat, aes(x=label,y=length))+geom_boxplot(aes(fill=label))+theme_bw()+ggtitle('length distribution(refined data)')+scale_fill_brewer(palette="Dark2")
pl2=ggplot(designMat, aes(x=label,y=length))+geom_violin(aes(fill=label))+theme_bw()+ggtitle('length distribution(refined data)')+scale_fill_brewer(palette="Dark2")
pl3=ggplot(designMat, aes(x=length, color=label))+geom_density()+theme_bw()+ggtitle('length distribution(refined data)')+scale_color_brewer(palette="Dark2")
grid.arrange(pl1, pl2, pl3, ncol=3,nrow=1)


designMat_base_norm <- cbind(designMat[,c('a','c','u','g')]/designMat$length, label=designMat$label)
base_p1=DrawFeatureDistribution_LabelBased(subset(designMat_base_norm, select=c(a,c,u,g,label)))
pheatmap(cor(subset(designMat_base_norm,select=c(a,c,u,g))))
